...
#fft_scheduler::test_swizzler();
swizzler::test_swizzler();
exit 0;
...
foreach my $npoints (8,16,32,64,128,256,512,1024) {
    foreach my $nunits (1,2,4) {
        ...
        fft_scheduler::fft_schedule($npoints, $nunits, "resched", "TEST");
    }
}
exit 0;

##############################################################################
# @fft_scheduler::fft_info = fft_scheduler::fft_schedule(16, 4, "reschedule", "");
# print "hello\n";
# print "\n";
# for (my $i=0; $i <  @fft_scheduler::fft_info; $i++) {
#     
#     my $bufnum1 = @fft_scheduler::fft_info[$i]->{op1_buffer};
#     my $bufnum2 = @fft_scheduler::fft_info[$i]->{op2_buffer};
#     my $bufacc1 = @fft_scheduler::fft_info[$i]->{op1_buffer_access};
#     my $bufacc2 = @fft_scheduler::fft_info[$i]->{op2_buffer_access};
#     
